          ...
          <div class="col-xl-10 col-lg-10 col-md-10 mt-md-0 mt-4">
            <div class="align-items-md-center row">
              <div class="col-xl-8 col-lg-7 col-sm-6 pr-lg-3 pr-md-0 user-profile">
                ...
                <p class="mb-0 mb-2 member-from">{{__('individualProfileView.memberSince')}} {{$customer->created_at->format('M-Y-D')}}</p>
                ...
                <div class="col-lg-9 col-md-9 col-sm-9 col-8 listingCar-descCar">
                  ...
                  <div class="align-items-center d-flex justify-content-between listingCar-place mb-3">
                    ...
                    <p class="themecolor2 font-weight-semibold mb-0 negotiable">
                      @if($show_result->neg)
                      {{__('individualProfileView.negotiable')}}
                      @endif
                    </p>
                  </div>
                  ...
                  <div class="align-items-center row contactRow ">
                    <div class="col-lg-5 col-md-5 col-sm-4 col-4 ">
                      <p class="mb-0"> @php
                        ...
                        @endphp {{__('individualProfileView.updated')}} {{$diff}} {{__('individualProfileView.daysAgo')}}</p>
                    </div>
                    <div class="align-items-center col-8 col-lg-7 col-md-7 col-sm-8 d-flex justify-content-end text-right pl-md-3 pl-0">
                      ...
                      @php $id = 'heart'; @endphp
                      @endif
                      <a href="javascript:void(0)" class="saveAd saveAdBtn mr-3" data-id="{{$show_result->id}}"><em id="{{$id}}{{$show_result->id}}" class="fa fa-heart {{$id}}"></em> <span class="save_heart">{{__('individualProfileView.save')}}</span></a>
                      @else
                      <a target="" href="{{url('user/login')}}" class="saveAd saveAdBtn mr-3" data-id="{{@$show_result->id}}"><em id="{{@$id}}{{$show_result->id}}" class="fa fa-heart {{@$id}}"></em> <span class="save_heart">{{__('individualProfileView.save')}}</span></a>
                      @endif
                      <a target="" href="{{url('user/login')}}" class="mr-3" data-id="{{@$show_result->id}}"><em class="fa fa-share" style="color: gray"></em> <span class="share_heart">{{__('individualProfileView.share')}}</span></a>

                      @if(Auth::guard('customer')->user())
                      <a data-customer="{{$show_result->customer_id}}" data-id="{{$show_result->id}}" data-ads_title="{{@$show_result->maker->title}} {{@$show_result->model->name}} {{@$show_result->year}} {{@$show_result->versions->name}}" href="javascript:void(0)" style="font-size: 12px;font-weight: 200;padding: 5px 5px;" class="btn contactbtn themebtn3 show_phone_no send-msg-btn"><em class="fa fa-envelope"></em> {{ __('individualProfileView.sendMessageButton') }}</a>
                      @else
                      <a target="" href="{{url('user/login')}}" class="btn contactbtn themebtn3 show_phone_no">
                        <em class="fa fa-envelope"></em> {{ __('individualProfileView.sendMessageButton') }}</a>
                      @endif
                    </div>
            ...
            <div class="p-md-4 p-3 notifyForm detail-page-notifyform" style=" background: #E4E4E4;">
              <div class="row align-items-center">
                <div class="col-lg-5 col-12 mb-lg-0 mb-3">
                  <h5 class="themecolor"><em class="fa fa-bell"></em> {{__('individualProfileView.notifyMe')}}</h5>
                  <p class="mb-0">{{__('individualProfileView.notifyMeDetailText1')}} <strong>{{@$customer->customer_company}}</strong> {{__('individualProfileView.notifyMeDetailText2')}} </p>
                </div>
                <div class="col-lg-7 col-12">
                  <form>
                    <div class="row form-row">
                      <div class="col-sm-5 mb-md-0 mb-3 form-group">
                        <input type="text" name="" placeholder="{{__('individualProfileView.emailAddress')}}" class="form-control">
                      </div>
                      <div class="col-sm-4 mb-md-0 mb-3 form-group">
                        <select name="selectCity" class="form-control">
                          <option value="">{{__('individualProfileView.daily')}}</option>
                          <option value="">{{__('individualProfileView.weekly')}}</option>
                          <option value="">{{__('individualProfileView.monthly')}}</option>
                          <option value="">{{__('individualProfileView.hourly')}}</option>
                        </select>
                      </div>
                      <div class="col-sm-3 mb-md-0 mb-3 form-group notifySubCol">
                        <input type="submit" value="{{__('individualProfileView.notifyBottonText')}}" class="btn btn-block border-0 text-white notifySubBtn ">
                      </div>
                    </div>
                  </form>
                </div>
