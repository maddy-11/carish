...
<div class="bg-white main-titl-row  pb-3 pt-3 row align-items-center ">
   ...
   <div class="col-lg-8 col-md-7 col-sm-7  title-col">
    <h3 class="fontbold maintitle mb-2 mb-sm-0 text-capitalize">Fuel Types</h3>
  </div>
  <div class="col-lg-4 col-md-5 col-sm-5 search-col text-right">
      <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal" style="float: right">Add Fuel Type</button>
  </div>
          ...
          <tbody>
            @foreach($engineTypes as $engine)
             <tr id="engine_type_id_{{$engine->id}}">
                ...
                <td>
                  @php
                   $p_caty = ($engine->engine_type_description()->where('language_id',2)->pluck('title')->first());
                   @endphp
                  {{$p_caty !== null ? @$p_caty : '--'}}
                </td>
...
<div class="modal fade" id="myModal" role="dialog">
   <div class="modal-dialog">
      <!-- Modal content-->
    <form action="{{url('add/engine_type')}}" method="POST" enctype="multipart/form-data">
      <div class="modal-content">
         ...
         <div class="modal-footer">
            ...
            <button type="submit" class="btn" style="color: white; background-color: #5a386b">Add Fuel Type</button>
            </div>
      </div>
    </form>
   </div>
...
<div class="modal fade" id="editmodal{{$engine->id}}" role="dialog">
   <div class="modal-dialog">
      <!-- Modal content-->
      <form action="{{url('edit/engine_type')}}" method="POST" enctype="multipart/form-data">
      <div class="modal-content">
         <div class="modal-header">
         ...
         <div class="modal-body">
           ...
           <input type="hidden" name="engine_type_id" id="engine_type_id">
         ...
         <div class="modal-footer">
         <button type="submit" class="btn" style="color: white; background-color: #5a386b">Edit Fuel Type</button>
         <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
         </div>
      </div>
      </form>
...
<script>
   ...
   $(function(e){
      ...
      swal({
          title: "Alert!",
          text: "Are you sure you want to Delete this Engine Type?",
          type: "info",
   ...
   $(document).on('click', '.editaction', function(){
      var id = $(this).data('id');
      // alert(id);
      $('#engine_type_id').val(id);
      $.ajax({
                method:"get",
                dataType:"json",
                data:{id:id},
                url:"{{ url('admin/edit-engine-type') }}",
                beforeSend:function(){
                   $('#loader_modal').modal({
                        backdrop: 'static',
                        keyboard: false
                  ...
                  $("#loader_modal").modal('hide');
                    ...
                    $('#editmodal').modal('show');
                     ...
                     if(data.spareparts.length > 0){
                      // alert('hi');
                      $('#engine_type_id').val(data.spareparts[0]['engine_type_id']);
