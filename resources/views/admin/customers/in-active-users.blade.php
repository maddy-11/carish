...
<div class="bg-white main-titl-row  pb-3 pt-3 row align-items-center ">
   ...
   <div class="col-lg-8 col-md-7 col-sm-7  title-col">
    <h3 class="fontbold maintitle mb-2 mb-sm-0 text-capitalize" id="main">Inactive / Business Users</h3>
  </div>
            ...
            @foreach($buisness as $user)
             <tr id="user_id_{{$user->id}}">
                <td>
                  ...
                  <a class="fa fa-eye" href="{{url('admin/in-active/user-detail/'.$user->id)}}" ><i title="View User"></i></a>
                   <a data-toggle="modal" href='#deleteModal' class="actionicon bg-danger deleteaction delete-btn" data-id="{{$user->id}}"><i class="fa fa-close"></i></a></div>
                </td>
          ...
          <tbody>
            @foreach($users as $user)
             <tr id="user_id_{{$user->id}}">
                ...
                <td><a href="{{url('admin/in-active/user-detail/'.$user->id)}}" ><b>{{$user->customer_company !== null ? $user->customer_company : '--'}}</b></a></td>
                <td>{{$user->customer_email_address}}</td>
                <td>{{$user->customers_telephone}}</td>
