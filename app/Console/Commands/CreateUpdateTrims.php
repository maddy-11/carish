...
class CreateUpdateTrims extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'makemodels:createtrims';
     ...
     * @var string
     */
    protected $description = 'this command create/updates trims tabel car_trim';
    ...
    public function handle()
    {
        ...
        try {
            if ($basebuyAutoApi->trimGetDateUpdate($idType) > $lastDateUpdate) {
                $downloadedFilePath = $basebuyAutoApi->trimGetAll($idType);
            }
                ...
                while (!feof($fp)) {
                    if (!$ignoreFirst) {
                        ...
                        if (!empty($fields) && !empty($fields[0]) && str_replace("'", "", $fields[3]) != 'name') {
                            if (str_replace("'", "", $fields[8]) == '1' || str_replace("'", "", $fields[8]) == 1) {
                                DB::table('car_trim')
                                    ->updateOrInsert(
                                        [
                                            'id_car_trim' => str_replace("'", "", $fields[0])
                                        ],
                                        [
                                            'id_car_serie' => str_replace("'", "", $fields[1]),
                                            'id_car_model' => str_replace("'", "", $fields[2]),
                                            'name' => str_replace("'", "", $fields[3]),
                                            'start_production_year' => str_replace("'", "", $fields[4]),
                                            'end_production_year' => str_replace("'", "", $fields[5]),
                                            'date_create' => str_replace("'", "", $fields[6]),
                                            'date_update' => str_replace("'", "", $fields[7]),
                                            'id_car_type' => str_replace("'", "", $fields[8])
                                        ]
                                    );
                            }
                        }
