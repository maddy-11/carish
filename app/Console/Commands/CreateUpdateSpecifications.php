...
class CreateUpdateSpecifications extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'makemodels:createspecifications';
     ...
     * @var string
     */
    protected $description = 'this command create/updates specifications tabel car_specification';
    ...
    public function handle()
    {
        ...
        try {
            if ($basebuyAutoApi->specificationGetDateUpdate($idType) > $lastDateUpdate) {
                $downloadedFilePath = $basebuyAutoApi->specificationGetAll($idType);
            }
                ...
                while (!feof($fp)) {
                    if (!$ignoreFirst) {
                        ...
                        if (!empty($fields) && !empty($fields[0]) && str_replace("'", "", $fields[1]) != 'name') {
                            if (str_replace("'", "", $fields[5]) == '1' || str_replace("'", "", $fields[5]) == 1) {

                                DB::table('car_specification')
                                    ->updateOrInsert(
                                        [
                                            'id_car_specification' => str_replace("'", "", $fields[0])
                                        ],
                                        [
                                            'name'        => str_replace("'", "", $fields[1]),
                                            'id_parent'   => str_replace("'", "", $fields[2]),
                                            'date_create' => str_replace("'", "", $fields[3]),
                                            'date_update' => str_replace("'", "", $fields[4]),
                                            'id_car_type' => str_replace("'", "", $fields[5])
                                        ]
                                    );
                            }
                        }
