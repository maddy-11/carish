...
class CreateUpdateGenerations extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'makemodels:creategenerations';
     ...
     * @var string
     */
    protected $description = 'this command create/updates generations tabel car_generation';
                    ...
                    if (!$ignoreFirst) {
                        ...
                        if (!empty($fields) && !empty($fields[0]) && str_replace("'", "", $fields[2]) != 'name') {
                            if (str_replace("'", "", $fields[7]) == '1' || str_replace("'", "", $fields[7]) == 1) {
                                DB::table('car_generation')
                                    ->updateOrInsert(
                                        [
                                            'id_car_generation' => str_replace("'", "", $fields[0])
                                        ],
                                        [
                                            'id_car_model' => str_replace("'", "", $fields[1]),
                                            'name' => str_replace("'", "", $fields[2]),
                                            'year_begin' => str_replace("'", "", $fields[3]),
                                            'year_end' => str_replace("'", "", $fields[4]),
